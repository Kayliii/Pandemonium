...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    MY_HIGHP_OR_MEDIUMP vec4 tex_dest = Texel( clip_texture, uv);
    ...
    vec4 result = dissolve_mask(tex, texture_coords, uv);
    //return vec4(result.rgb, tex_dest.a);
    return vec4(result.rgb, min(result.a, tex_dest.a));
    //return result.rgba * 0.01 + tex_dest.rgba;
}
